-- Migration: Corrigir cálculo de valor das parcelas para receitas
-- Descrição: Para receitas, as parcelas devem ser calculadas usando valor_liquido (não valor_bruto)
-- Problema: Trigger estava usando valor_bruto para ambos, mas receitas têm taxas que devem ser deduzidas

-- 1. Remover trigger existente
DROP TRIGGER IF EXISTS trigger_gerar_parcelas ON transacoes;

-- 2. Recriar função com cálculo correto para receitas (valor_liquido) e despesas (valor_bruto)
...
DECLARE
  ...
  v_valor_base NUMERIC(10,2); -- Usar valor_liquido para receitas, valor_bruto para despesas
  i INTEGER;
BEGIN
  v_num_parcelas := COALESCE(NEW.total_parcelas, 1);
  
  -- Para receitas, usar valor_liquido (já deduzidas as taxas)
  -- Para despesas, usar valor_bruto (não há taxas)
  IF NEW.tipo = 'receita' THEN
    v_valor_base := NEW.valor_liquido;
  ELSE
    v_valor_base := NEW.valor_bruto;
  END IF;
  
  -- Calcular valor de cada parcela (distribuir centavos na última)
  v_valor_parcela := TRUNC(v_valor_base / v_num_parcelas, 2);
  v_valor_restante := v_valor_base - (v_valor_parcela * (v_num_parcelas - 1));
...
COMMENT ON FUNCTION gerar_parcelas_transacao() IS 'Gera parcelas automaticamente. Para receitas usa valor_liquido (com taxas deduzidas), para despesas usa valor_bruto (sem taxas).';

-- 5. Corrigir parcelas existentes de receitas (opcional - apenas se necessário)
-- ATENÇÃO: Isso vai recalcular todas as parcelas existentes de receitas
-- Descomente apenas se precisar corrigir dados existentes
...
DECLARE
  ...
  i INTEGER;
BEGIN
  -- Para cada transação de receita com parcelas
  FOR rec IN 
    SELECT id, valor_liquido, total_parcelas, data_vencimento
    FROM transacoes 
    WHERE tipo = 'receita' 
    AND total_parcelas > 1
  LOOP
    -- Remover parcelas existentes
    DELETE FROM parcelas WHERE transacao_id = rec.id;
    
    -- Recalcular valores
    v_num_parcelas := rec.total_parcelas;
    v_valor_parcela := TRUNC(rec.valor_liquido / v_num_parcelas, 2);
    v_valor_restante := rec.valor_liquido - (v_valor_parcela * (v_num_parcelas - 1));
    v_data_base := COALESCE(rec.data_vencimento, CURRENT_DATE);
    
    -- Recriar parcelas com valores corretos
    FOR i IN 1..v_num_parcelas LOOP
      INSERT INTO parcelas (
        clinica_id,
        transacao_id,
        ...
        (SELECT clinica_id FROM transacoes WHERE id = rec.id),
        rec.id,
        i,
        v_num_parcelas,
        CASE WHEN i = v_num_parcelas THEN v_valor_restante ELSE v_valor_parcela END,
        v_data_base + ((i - 1) * INTERVAL '1 month'),
        'pendente'
      );
    END LOOP;
  END LOOP;
